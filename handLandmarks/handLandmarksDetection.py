...
if __name__ == "__main__":
    # Example usage
    image_path = Path(r"C:\Users\Staff\OneDrive - University of Greenwich\HandsDatasets\11kHands\Hands\Hand_0000512.jpg")
    ...
    norm_lndmrk,_ = detector.detect(img)

    print("Landmarks array shape:", None if norm_lndmrk is None else norm_lndmrk.shape)
    print("First few landmarks:\n", None if norm_lndmrk is None else norm_lndmrk[:5])

    if norm_lndmrk is not None:
        DisplayUtils.display_with_coords(img, norm_lndmrk)
    else:
    ...
    # # 4) Draw landmarks (green) and bounding box (cyan) directly on full-res image
    # for x, y, _ in landmarks.astype(int):
    #     cv2.circle(img, (x, y), 5, (0, 255, 0), -1)

    # xmin, ymin, xmax, ymax = map(int, box)
    # cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (255, 255, 0), 2)

    # # 5a) Display with OpenCV window at full size
    # cv2.namedWindow("Full-Res Hand Landmarks", cv2.WINDOW_NORMAL)
    # cv2.imshow("Full-Res Hand Landmarks", img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    # # — or — 

    # # 5b) Save to disk and open in your favorite viewer
    # cv2.imwrite("full_res_landmarks.png", img)
    # print("Saved: full_res_landmarks.png")